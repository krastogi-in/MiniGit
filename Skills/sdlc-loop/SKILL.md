...
| Ideate + spec + plan all present | add **`sdlc:agent-ready`**; post **Jira comment** with idea/spec/tasks (do **not** overwrite Description) | **In Progress** |
...
Evidence: Jira **comment** “SDLC review package” (idea/spec/tasks) and/or PR URL; agent_space/<KEY>/ as backup
Labels now: …
Unblock: add label `sdlc:human-ready` OR comment `approved`
(For plan stage, `sdlc:agent-ready` must already be present; review-package **comment** must exist. Never overwrite issue Description.)
```
...
2. Post a **new** Jira comment with the revised review package; keep `sdlc:agent-ready`; do **not** invent `sdlc:human-ready`; do **not** overwrite Description.
