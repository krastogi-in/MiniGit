...
You stay on the **outer loop**. Agents run the **inner loop**. Jira is the **state database**: `sdlc:*` progress labels, `aiagent-*` / `human-approved` gates, comments, and workflow status.
...
3. After plan: look for `aiagent-ready` + **In Progress**; add `human-approved` (or comment `approved`).
4. Review the PR on `aiagent/<KEY>`; agent sets `aiagent-approved`, then status **Review** after you accept.
5. Optional: add `aiagent-need-review-stage` for feedback incorporation.
