...
**Input:** any Jira issue. **State:** accumulated `sdlc:*` + gate labels (`aiagent-*`, `human-approved`) + comments + status.  
...
3. **HUMAN GATE (plan)** — after `aiagent-ready` + In Progress; unblock with `human-approved` or comment `approved`
4. **Maker + PR** — branch `aiagent/<KEY>`; label `aiagent-approved`
5. **HUMAN GATE (PR)** — then status **Review**
6. **Checker** — inline comments; honor `aiagent-need-review-stage` (agent strips stale `human-approved` / `aiagent-approved`, then waits for re-approval)
...
| Missing gate | Maker attempted without `aiagent-ready` / `human-approved` |
